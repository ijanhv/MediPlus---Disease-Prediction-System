...
export default function Modal({ showModal, setShowModal, text }: ModalProps) {
    ...
    useEffect(() => {
        if (text === 'The person is not diabetic') {
            ...
            setEmoji('😊')
        } else if (text == 'The person is diabetic') 
        {
                                ...
                                <div className="flex items-center justify-end p-6 border-t border-solid border-slate-200 rounded-b">
                                    ...
                                    {text !== 'The person is not diabetic' &&
                                        (<button
                                            ...
                                            onClick={() => setShowModal(false)}
                                        >
                                            <Link href={`/doctors-near?speciality=Diabetologist&city=${city}`}>Find Diabetalogists near you</Link>
                                        </button>)
                                    }
                                </div>
                            </div>
